...
Guess Number Game
idea: only use a variabler to store curren't guess. Everything else will be handled by recursion
|#
...
(define (get-answer message correct_answer num_tries)
	...
	(let* 
		(
			[guess 0] ;will be used to store my guess for this run.
		)
		(set! guess (read)) ;Read the user's guess.
